...
由于各种模板引擎的语法各不相同，以下标签语法仅作为参考。

* `@import("静态资源ID")` 

  用来加载某一静态资源，通过 `静态资源ID` 指定。

  fis 中所有资源都可以根据配置随意部署到各种目录，或者 cdn 服务器上，最终产出的路径是相对不固定的。对于开发人员来说，使用产出路径是不可靠的，所以需要通过`静态资源ID`来指定资源。`静态资源ID` 是相对固定的，他的值为该资源在项目中的绝对路径。（注意：没有开头的斜线`/`）如：
  ...
  * `common:static/css/global.css`
  ...
  除了加载资源本身外，还应当进一步分析该资源依赖表，递归加载所有依赖，减少人工分析成本。这也是对 fis [三种语言能力](http://fis.baidu.com/fis3/docs/user-dev/extlang.html)中[声明依赖](http://fis.baidu.com/fis3/docs/user-dev/require.html)能力的落实。
  ...
  此类路径，需要借助编译工具，交给后端程序时应该被替换成 `静态资源ID`。
* `@url("静态资源ID")` 
...
1. 扩展模板语法糖，详情请见模板语法糖扩展部分。
2. 基于静态资源表的资源管理。详情请见模块化开发部分。
